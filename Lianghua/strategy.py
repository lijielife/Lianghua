...
class mystrategy(StrategyBase):
    ...
    def handle_data(self):
        ...
        if isnan(m1[-1]) or isnan(m2[-1]):  # 排除空值
            ...
            print
            self.account.referencePortfolioValue
            # print self.botter


if __name__ == '__main__':
    my = mystrategy(start='2011-01-01', end='2015-01-01', hist=30, banchmark='HS300', universe='A',
                    capital_base=10000000, commission=[0.001, 0.002], slippage=0)  # 初始化策略类
    my.run()  # 执行回测
    # start 开始日期
    # end 结束日期
    # hist 历史数据长度
